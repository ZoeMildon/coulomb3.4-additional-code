...
% Calculates the average Coulomb stress for a whole fault plane
% (when the source/receiver faults are comprised of multiple elements) and
% the percentage of the individual fault surfaces that are positively
% stressed
...
disp('Select the .csv file')
    ...
    fprintf(fid,'%s \t %s \t %s\n','Fault name','Average CST (bars)','% positive');
    ...
    for i=1:length(fault_names)
        ...
        fprintf(fid,'%s \t %f \t %2.1f\n',fault_name,mean_coulomb,perc_positive);
    end
